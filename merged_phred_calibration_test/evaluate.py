...
def process_reads(orig, read1, read2, merge_df):
    phred_counter = dict()
...
def main(template_path, s1_path, s2_path, merge_info_path, nfrags, fraglen, qualityshift, tool_name,
    ...
    phred_counter = dict()
    reads_for_multithread = []
    for i in range(nfrags):
        reads_for_multithread.append(
            get_next_entries(templates_file, s1_file, s2_file, fraglen, 
                             dna_complement)
            )
        
        if (i+1) % 100 == 0:
            # parse 100 reads to the multithread function
            phred_counter_list = Parallel(n_jobs=-1)(
                delayed(process_reads)(orig, read1, read2, merge_df) \
                for orig, read1, read2 in reads_for_multithread
                )
            # merge the singlethread counter into the total counter
            merge_singlethread_counter(phred_counter_list, phred_counter)
            # reset 
            reads_for_multithread = []
    # multithread for the last reads
    phred_counter_list = Parallel(n_jobs=-1)(
        delayed(process_reads)(orig, read1, read2, merge_df) \
        for orig, read1, read2 in reads_for_multithread
        )
